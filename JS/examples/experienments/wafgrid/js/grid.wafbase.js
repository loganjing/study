...
$.extend($.jgrid,{
	...
	getAccessor : function(obj, expr) {
		...
		return $.jgrid.getMultiValue(ret); //modiby by jlg.
	},
...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
		...
		var p = $.extend(true,{
			...
			cmTemplate : {},
			idPrefix : "",
            loadInit:true, //add by jlg,
            saveWhenSubmit:true,//add by jlg,
            entryProp:"entrys",//add by jlg,
            tabindex:"0",//add by jlg,
            saveAllHidden:false
		}, $.jgrid.defaults, pin || {});
		var ts= this,grid={
			headers:[],
		...
		$(this).empty().attr("tabindex",p.tabindex); //modiby by jlg.
		this.p = p ;
		...
		formatter = function (rowId, cellval , colpos, rwdat, _act){
            ...
            if(ts.p.cellEdit === true&&$.isFunction(ts.p.formatCellEditor)) {
                ...
                cellEditorConfig = $("#"+ts.p.id).jqGrid("getCellEditorConfig",rowId,cm.name);
            }
			if(typeof cm.formatter !== 'undefined' || (cellEditorConfig&&cellEditorConfig.formatter)) {
				...
				if($.isFunction( cm.formatter ) ) {
                    ...
                    //end by jlg.
					v = $.fn.fmatter(formatterStr, cellval,opts, rwdat, _act,ts.p.id);
				} else {
					v = cellVal(cellval);
				}
			} else {
		...
		endReq = function() {
            ...
            //add by jlg.
            if($.isFunction(ts.p._endReq)) {ts.p._endReq.call(ts);}
		},
				...
				case "script":
					$.ajax($.extend({
						...
						beforeSend: function(xhr, settings ){
                            if(ts.p.loadInit===false || ts.p.loadInit === "false") return false; //add by jlg
							var gotoreq = true;
		...
		setPager = function (pgid, tp){
			...
			if(ts.p.rowList.length >0){
				str = "<td dir='"+dir+"'>";
                str +=$.wafjgrid.pager.rowListCaption+"";//add by jlg.
				str +="<select class='ui-pg-selbox' role='listbox'>";
                //add by jlg
                var find = false;
				for(i=0;i<ts.p.rowList.length;i++){
					str +="<option role=\"option\" value=\""+ts.p.rowList[i]+"\""+((ts.p.rowNum == ts.p.rowList[i])?" selected=\"selected\"":"")+">"+ts.p.rowList[i]+"</option>";
                    //add by jlg.如果默认的rowNum不再下拉列表中，默认取下拉列表的第一个.
			...
			if(ts.p.pgbuttons===true) {
                ...
                pgl += pginp !== "" ? pginp:"";//add by jlg.
                pgl += "<td id='"+po[0]+"' class='ui-pg-button ui-corner-all' title='"+$.wafjgrid.pageNav.first+"'><span class='ui-icon ui-icon-seek-first'></span></td>";
				pgl += "<td id='"+po[1]+"' class='ui-pg-button ui-corner-all' title='"+$.wafjgrid.pageNav.prev+"'><span class='ui-icon ui-icon-seek-prev'></span></td>";
                //remove by jlg.
				pgl += "<td id='"+po[2]+"' class='ui-pg-button ui-corner-all' title='"+$.wafjgrid.pageNav.next+"'><span class='ui-icon ui-icon-seek-next'></span></td>";
				pgl += "<td id='"+po[3]+"' class='ui-pg-button ui-corner-all' title='"+$.wafjgrid.pageNav.end+"'><span class='ui-icon ui-icon-seek-end'></span></td>";
		...
		ts.p.xmlReader = $.extend(true,{
			...
			id: "[id]",
			userdata: "userdata",
			subgrid: {root:"rows", row: "row", repeatitems: false , cell:"cell"}//modiby default value = false, by jlg
		}, ts.p.xmlReader);
					...
					$('td[role="gridcell"]',ind).each( function(i) {
						...
						if ( nm !== 'cb' && nm !== 'subgrid' && nm !== 'rn') {
							if($t.p.treeGrid===true && nm == $t.p.ExpandColumn) {
								...
								try {
									res[nm] = $.unformat.call($t,this,{rowId:ind.id,colModel:$t.p.colModel[i],gid:$t.p.id},i);//modiby by jlg
								} catch (e){
			...
			if(pos>=0) {
				...
				if (ind){
					...
					if(nData !== "" || forceupd === true) {
						...
						v = $t.formatter(rowid, nData, pos,rowdata,'edit');
	...
	getCell : function(rowid,col) {
		...
		this.each(function(){
			...
			if(pos>=0) {
				...
				if(ind) {
					try {
						ret = $.unformat.call($t,$("td:eq("+pos+")",ind),{rowId:ind.id, colModel:$t.p.colModel[pos],gid:$t.p.id},pos); //modiby by jlg
					} catch (e){
				...
				if (ln && ln>0){
					while(i<ln){
						if($($t.rows[i]).hasClass('jqgrow')) {
							try {
								val = $.unformat.call($t,$($t.rows[i].cells[pos]),{rowId:$t.rows[i].id, colModel:$t.p.colModel[pos],gid:$t.p.id},pos);//modify by jlg
							} catch (e) {

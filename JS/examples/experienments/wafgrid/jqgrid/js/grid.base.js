		...
		formatter = function (rowId, cellval , colpos, rwdat, _act){
			...
			if(typeof cm.formatter !== 'undefined' || (cellEditorConfig&&cellEditorConfig.formatter)) {
				...
				if($.isFunction( cm.formatter ) ) {
                    ...
                    //end by jlg.
					v = $.fn.fmatter(formatterStr, cellval,opts, rwdat, _act);
				} else {
					v = cellVal(cellval);
				}
			} else {
					...
					$('td[role="gridcell"]',ind).each( function(i) {
						...
						if ( nm !== 'cb' && nm !== 'subgrid' && nm !== 'rn') {
							if($t.p.treeGrid===true && nm == $t.p.ExpandColumn) {
								...
								try {
									res[nm] = $.unformat.call($t,this,{rowId:ind.id,gid:$t.p.id, colModel:$t.p.colModel[i]},i);//modiby by jlg
								} catch (e){
	...
	getCell : function(rowid,col) {
		...
		this.each(function(){
			...
			if(pos>=0) {
				...
				if(ind) {
					try {
						ret = $.unformat.call($t,$("td:eq("+pos+")",ind),{rowId:ind.id,gid:$t.p.id, colModel:$t.p.colModel[pos]},pos); //modiby by jlg
					} catch (e){
				...
				if (ln && ln>0){
					while(i<ln){
						if($($t.rows[i]).hasClass('jqgrow')) {
							try {
								val = $.unformat.call($t,$($t.rows[i].cells[pos]),{rowId:$t.rows[i].id,gid:$t.p.id, colModel:$t.p.colModel[pos]},pos);//modify by jlg
							} catch (e) {
